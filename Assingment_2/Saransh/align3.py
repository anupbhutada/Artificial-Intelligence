...
def Minimax(tile,a,depth,limit):
	max_depth=limit
	p1=a
	global count
	count+=1
	...
	if(tile.goal_test == True or (tile.green_color+tile.blue_color==tiles) or depth>=limit):
		...
		if depth % 2 == 1:
			...
			if not (available_moves):
				return 01,tile
			goal = max([Minimax(t, p1, depth, max_depth) for t in available_moves], key=lambda x : x[0])
			goal[1].append(tile)
			...
			if not (available_moves):
				return -10,tile
			goal = min([Minimax(t, p1, depth, max_depth) for t in available_moves], key=lambda x : x[0])
			goal[1].append(tile)
			return(goal[0],goal[1])
...
#starting the code execution time here
start=time.clock()

while(current_tile.goal_test == False):
	limit=16
	if (Player==blue):
		final_arr = Minimax(current_tile, blue, 0, limit)[1]
		...
		print("Enter column number(0,1,2,3)")
